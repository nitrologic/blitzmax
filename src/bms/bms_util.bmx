...
Function CompileCCmd$( src$,obj$,opts$ )
	DeleteFile obj
...
'	If Sys( cmd )
'		Throw "BMK Build Error: failed to compile "+src
'	EndIf
End Function

Function CompileBMXCmd$( src$,obj$,opts$ )
...
?Linux	
	Local cmd$=CQuote(BlitzMaxPath()+"/bin/bcc")+opts+" -o "+CQuote(azm)+" "+CQuote(src)
?
	Return cmd + " " + AssembleCmd(azm,obj)

End Function

Function CreateArcCmd$( path$ , oobjs:TList )
	Local cmd$,t$
?Win32
	cmd="ar -r "+CQuote(path)
	For t$=EachIn oobjs
		cmd:+" "+CQuote(t)
	Next
?MacOS
