...
'		If opt_release cc_opts:+" -O2 -DNDEBUG"
		...
		If MaxTime( obj_files )>arc.time Or opt_all
			...
			Local cmd$ = CreateArcCmd(arc_path,FilePaths( obj_files ))
	...
	If buildit And Match( src_ext,"bmx;c;m;cpp;cxx;cc;mm;s;asm" )
		...
		If src.time>FileTime( obj_path ) Or opt_all
			...
			Case "bmx"
				...
				cmd=CompileBMXCmd(src_path,obj_path,opts)
			Case "c","m","cpp","cxx","cc","mm"
				cmd=CompileCCmd(src_path,obj_path,cc_opts)
			Case "s","asm"
				cmd=AssembleCmd(src_path,obj_path)
			End Select
		EndIf
